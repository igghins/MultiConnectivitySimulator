...
% Downlink simulator for  Single-Connectivity.
...
% generated following FTP model 3.
...
% Output variables
result_throughput = cell(numIterations,1);
result_bler = cell(numIterations,1);
result_usage = cell(numIterations,1);
...
for it = 1:numIterations
    ...
    projections = Proj;
    %% Init simulation
    throughput = zeros(Nue,1);
    actualBLERperUser = zeros(Nue,1);
    ...
    for b = 1:Nbs
        ...
        if ~isempty(usersAtBS)
            ...
            for j = 1:length(usersAtBS)
                ...
                if isempty(auxSNR)
                    actualBLERperUser(usersAtBS(j)) = min(auxBler(:,2)); % Out of BLER curves
                else
                    actualBLERperUser(usersAtBS(j)) = auxBler(auxSNR,2);
                end
            end
            ...
            availableRatePerUser = RBperUser .* MbpsperRB(usersAtBS,b);
            throughput(usersAtBS) = availableRatePerUser .* (1 - actualBLERperUser(usersAtBS));
        end
    end
    result_throughput{it} = single(throughput);
    result_bler{it} = single(actualBLERperUser);
    result_usage{it} = single(listRB);
    if mod(it,100)==0
